"use client"

import { useEffect, useRef } from "react"
import Link from "next/link"
import { gsap } from "@/lib/gsap"

type Experience = {
  company: string
  role: string
  period: string
  summary: string
  stack: string[]
  bullets: string[]
}

const experiences: Experience[] = [
  {
    company: "Dassault Systèmes (EMEA HQ)",
    role: "Site Reliability Engineer - Apprentice",
    period: "Sep 2023 — Present",
    summary:
      "Engineering global Disaster Recovery, risk analysis tools, and observability pipelines to ensure IS&IT compliance and resilience.",
    stack: ["Python", "ML Clustering", "AWS", "Zero-Trust", "Docker", "Kubernetes", "Grafana", "JSONata", "ISO 27001", "SOC2"],
    bullets: [
      "Engineered the global Disaster Recovery orchestration framework to systematize worldwide IS&IT validation, improving test coverage by 25% across 200+ applications, including core systems driving €1B+ in annual revenue.",
      "Owned the global enterprise risk analysis process and tool. Re-engineered core algorithms via Python and ML clustering, scaling evaluation across 500+ platforms serving 1M+ users to ensure ISO 27001/SOC2 compliance.",
      "Architected an automated, zero-trust AWS Glacier backup system leveraging client-side encryption to isolate highly sensitive legal documentation, securing 100GB of critical corporate assets with a guaranteed 4-hour RTO.",
      "Engineered Python observability pipelines utilizing Grafana and JSONata to integrate with enterprise tooling, delivering real-time dashboards to executive management.",
    ],
  },
  {
    company: "Dassault Systèmes (NA HQ)",
    role: "Process Analyst and IT Operations – Intern",
    period: "Jun 2024 — Aug 2024",
    summary:
      "Optimized enterprise IT lifecycle workflows, secure decommissioning, and cross-domain Active Directory disruptions.",
    stack: ["Workflow Automation", "BitLocker", "Active Directory", "Security Audits", "Dashboards"],
    bullets: [
      "Led a workflow automation initiative for the secondhand IT asset lifecycle, engineering compliance audit dashboards and streamlining manual provisioning processes to accelerate hardware processing times by 30%.",
      "Executed the secure provisioning and decommissioning of enterprise hardware, enforcing strict BitLocker encryption, data sanitization, and recycling protocols to eliminate physical security vulnerabilities.",
      "Resolved critical cross-domain service disruptions for high-priority business units, executing advanced Active Directory identity validations and cryptographic key recoveries to maintain operational availability.",
    ],
  },
  {
    company: "INTM groupe",
    role: "Business Developer",
    period: "Jan 2023 — Jun 2023",
    summary:
      "Drove CAC40 business opportunities and needs analysis in AI/ML, Cybersecurity, and Data/Cloud, while building data infrastructure.",
    stack: ["CRM", "Lead Generation", "Needs Analysis", "AI/ML", "Cybersecurity", "Cloud"],
    bullets: [
      "Drove lead generation through extensive cold-calling and proactive outreach resulting in over 10 business opportunities with major clients, including those in the CAC40 index.",
      "Conducted high-level needs analysis on client projects related to AI/ML, Cybersecurity and Data/Cloud, effectively matching client requirements with internal technical expertise.",
      "Designed and delivered a custom CRM framework for ~5,000 prospects to streamline collaboration and workflows across the team.",
    ],
  },
]

export function Experiences() {
  const ref = useRef<HTMLDivElement | null>(null)

  useEffect(() => {
    if (!ref.current) return
    const cards = ref.current.querySelectorAll(".xp-card")
    gsap.set(cards, { y: 30, opacity: 0 })
    gsap.to(cards, {
      y: 0,
      opacity: 1,
      duration: 0.6,
      ease: "power2.out",
      stagger: 0.08,
      scrollTrigger: { trigger: ref.current, start: "top 80%", once: true },
    })
  }, [])

  return (
    <section id="experiences" className="w-full border-t">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-16 sm:py-20 md:py-24" ref={ref}>
        <h2 className="mb-8 text-2xl sm:text-3xl font-semibold tracking-tight">Experiences</h2>
        <div className="grid gap-5 md:grid-cols-2">
          {/* Plus card first to appear top-left */}
          <Link
            href="#contact"
            aria-label="Open to cutting-edge Computer Science opportunities"
            className="xp-card group rounded-2xl border border-primary/30 bg-[color-mix(in_oklch,var(--primary)_12%,transparent)] p-6 shadow-sm hover:shadow-md transition-all hover:border-primary/60 focus:outline-none focus:ring-2 focus:ring-[color-mix(in_oklch,var(--primary)_50%,transparent)] flex flex-col"
          >
            <div className="mb-3 flex items-center justify-between">
              <div className="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-[color-mix(in_oklch,var(--primary)_22%,transparent)] text-[color-mix(in_oklch,var(--primary)_92%,white_8%)]">
                <span className="text-xl leading-none">+</span>
              </div>
              <span className="text-xs text-[color-mix(in_oklch,var(--primary)_80%,white_20%)]">Let’s talk</span>
            </div>
            <h3 className="text-lg font-semibold tracking-tight text-[color-mix(in_oklch,var(--primary)_92%,white_8%)]">Open to cutting-edge work</h3>
            <p className="mt-2 text-sm leading-relaxed text-[color-mix(in_oklch,var(--primary)_85%,white_15%)]">
              Infrastructure, Cybersecurity, ML/AI, distributed systems, and more. I'm excited to join ambitious teams and projects.
            </p>
          <div className="mt-auto inline-flex items-center gap-2 text-sm font-medium text-[color-mix(in_oklch,var(--primary)_92%,white_8%)]">
              Contact me <span className="transition-transform group-hover:translate-x-0.5">→</span>
            </div>
          </Link>

          {experiences.map((xp) => (
            <article key={xp.company} className="xp-card rounded-2xl border bg-card text-card-foreground p-6 shadow-sm hover:shadow-md transition-all">
              <div className="flex items-center justify-between mb-1">
                <h3 className="text-lg font-semibold tracking-tight">{xp.role}</h3>
                <span className="text-xs text-muted-foreground">{xp.period}</span>
              </div>
              <div className="text-sm text-muted-foreground mb-3">{xp.company}</div>
              <p className="text-sm leading-relaxed mb-4 text-muted-foreground">{xp.summary}</p>
              <ul className="mb-4 space-y-1.5 text-sm text-muted-foreground">
                {xp.bullets.map((b, i) => (
                  <li key={i} className="flex items-start gap-2"><span className="mt-2 h-1 w-3 rounded-full bg-primary/70" /> <span>{b}</span></li>
                ))}
              </ul>
              <div className="flex flex-wrap gap-2">
                {xp.stack.map((s) => (
                  <span key={s} className="rounded-full border px-2.5 py-1 text-xs text-muted-foreground">
                    {s}
                  </span>
                ))}
              </div>
            </article>
          ))}
          {/* Removed duplicate plus card */}
        </div>
      </div>
    </section>
  )
}
